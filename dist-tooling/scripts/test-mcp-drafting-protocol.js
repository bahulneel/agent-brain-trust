...
async function main() {
    ...
    const client = new Client({ name: "drafting-protocol-test", version: "0.0.1" }, { capabilities: {} });
    ...
    const resolvedJson = JSON.parse(resolvedText);
    ...
    const expertJson = JSON.parse(firstText(experts));
    ...
    const pick = firstHit.expert_ids?.[0] ??
        (searchJson.hits
            ?.map((h) => h.item)
    ...
    const refsJson = JSON.parse(firstText(refs));
...
//# sourceMappingURL=test-mcp-drafting-protocol.js.map