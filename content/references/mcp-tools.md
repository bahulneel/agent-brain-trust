...
| `resolve_topics` | **Multi-query** resolution: `merge` (vote across `query` + `queries[]`), `intersect` (topic must hit every query), or `converge` (merge + fixed-point refinement from top hit). Prefer when you have several task keywords; see skill **`draft-experts`** |
| `search_topics` | Fuzzy search over the topic index (`topics-search.json`): labels, ids, aliases, keywords; **catch-all** progressive discovery when `resolve_topics` is too narrow or returns weak hits |
...
1. **`resolve_topics`** when you have **several short task phrases** and want a ranked merge, strict intersect, or converge-to-fixed-point result; then **`get_expert`** for chosen ids.
2. **`search_topics`** for **broad or exploratory** fuzzy lookup; **`get_topic_taxonomy`** to browse branches when search is ambiguous or empty.
3. Resolve **expert ids** at leaves, then **`get_expert`** (or **`get_experts_rost`** once if you need everyone). Prefer **exact id** when the user names it.
4. Call `list_references` once per session if you need rules not already loaded.
5. `get_reference` for **INDEX.md** first if you need a route map; then fetch only what applies (e.g. **dialogue.md** during Grounding).
