...
**Plugin `.mcp.json` (generated by the build):** both Cursor and Claude plugin trees ship the same stdio config — **`command`: `node`**, **`args`**: **`${CLAUDE_PLUGIN_ROOT}/scripts/mcp-server.cjs`**, and **`env.BRAIN_TRUST_RESOURCES`**: **`${CLAUDE_PLUGIN_ROOT}/resources`**. Cursor and Claude Code expand **`${CLAUDE_PLUGIN_ROOT}`** to the plugin install path. The standalone npm package still documents **`npx -y @bahulneel/brain-trust-mcp`** for clients that are not using a plugin zip.

**Contributors:** this repo does **not** ship a project-level Cursor MCP file (avoids duplicating the plugin’s launch story). After **`npm run build`**, use the **installed plugin** (its **`.mcp.json`**), **`npx -y @bahulneel/brain-trust-mcp@…`** for the published package, or add a **user-level** MCP server in Cursor settings if you need to run **`node`** on **`packages/brain-trust-mcp/dist/brain-trust-mcp.js`** with **`BRAIN_TRUST_RESOURCES`** pointing at **`packages/brain-trust-mcp/resources`**.
