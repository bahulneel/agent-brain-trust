...
**Topics:** the hierarchical expert index is the **rooted** tree under `assets/topics/knowledge-work/` (plus build-time **`topics-search.json`**). Use MCP `resolve_topics` (fast multi-query) then `search_topics` / `get_topic_taxonomy` as needed — or CLI `resolve-topics` / `search-topics` / `get-topic-taxonomy`. Skill **`draft-experts`** documents topic → expert id resolution.
