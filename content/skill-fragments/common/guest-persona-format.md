### Guest Persona Format

Every guest receives a lightweight profile **in full** before they participate or before the phase that requires them — incomplete rows are not valid:

- **Name & Epithet**: e.g. "Bryan Cantrill — Systems Realist" (always use the guest's full name)
- **Why drafted**: one sentence naming the specific gap this guest fills.
- **Approach**: the philosophy or lens they bring (not just "knows about X").
- **Core move**: their characteristic intellectual manoeuvre.
- **Signature question**: the question they'd ask first.
