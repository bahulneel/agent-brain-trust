...
**Plugin `.mcp.json` (generated by the build):** both plugin trees use **`command`: `node`** and the bundled **`scripts/mcp-server.cjs`**, with **`BRAIN_TRUST_RESOURCES`** pointing at the plugin **`resources/`** directory. Paths differ by host: Cursor uses **`${CURSOR_PLUGIN_ROOT}`**; Claude Code uses **`${CLAUDE_PLUGIN_ROOT}`**. The standalone npm package still documents **`npx -y @bahulneel/brain-trust-mcp`** for clients that are not using a plugin zip.
