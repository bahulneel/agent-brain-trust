...
type Leaf = { id: string; label: string; experts: string[]; keywords?: string[] };

/** Branch `topic.yml` body: ids and children come from directory layout. */
function branchMeta(label: string): Record<string, unknown> {
  return { label };
}
...
function leaf(l: Leaf): Record<string, unknown> {
  ...
  if (l.keywords?.length) o.keywords = l.keywords;
  return o;
}

async function writeBranch(rel: string[], data: Record<string, unknown>): Promise<void> {
  const dir = join(TOPICS, ROOTED_TOPIC_CLADE, ...rel);
  await mkdir(dir, { recursive: true });
  await writeFile(join(dir, "topic.yml"), stringify(data, { lineWidth: 0 }) + "\n", "utf8");
}

async function writeLeaf(rel: string[], l: Leaf): Promise<void> {
  const dir = join(TOPICS, ROOTED_TOPIC_CLADE, ...rel);
  await mkdir(dir, { recursive: true });
  await writeFile(join(dir, `${l.id}.yml`), stringify(leaf(l), { lineWidth: 0 }) + "\n", "utf8");
}

function expertStub(id: string): string {
  const title = id
    .split("-")
...
async function main(): Promise<void> {
  for (const id of NEW_EXPERTS) {
    if (EXISTING.has(id)) throw new Error(`duplicate expert id: ${id}`);
  }

  const entries = await readdir(TOPICS).catch(() => []);
  for (const name of entries) {
    if (name === ROOTED_TOPIC_CLADE) continue;
  ...
  for (const l of sw) await writeLeaf(["computing", "software-systems"], l);
  ...
  for (const l of pl) await writeLeaf(["computing", "programming-languages"], l);
  ...
  for (const l of hci) await writeLeaf(["design", "human-computer-interaction"], l);
  ...
  for (const l of vis) await writeLeaf(["design", "visual-communication"], l);
  ...
  for (const l of tw) await writeLeaf(["writing", "technical-writing"], l);
  ...
  for (const l of jn) await writeLeaf(["writing", "journalism-narrative-nonfiction"], l);
  ...
  for (const l of rs) await writeLeaf(["writing", "rhetoric-style-prose"], l);
  ...
  for (const l of te) await writeLeaf(["editing", "technical-editing"], l);
  ...
  for (const l of se) await writeLeaf(["explanation", "science-explanation"], l);
  ...
  for (const l of ds) await writeLeaf(["explanation", "demonstration-showing"], l);
  ...
  for (const l of ped) await writeLeaf(["education", "pedagogy"], l);
  ...
  for (const l of ps) await writeLeaf(["product", "product-strategy"], l);
  ...
  for (const l of pd) await writeLeaf(["product", "product-discovery"], l);
  ...
  for (const l of od) await writeLeaf(["organisation", "organisation-design"], l);
  ...
  for (const l of mc) await writeLeaf(["organisation", "management-collaboration"], l);
  ...
  for (const L of allLeaves) {
    for (const e of L.experts) referenced.add(e);
  }
